...
interface OtherAssumptionsSectionConfig {
  key: string;
  legend: string;
...
@Component({
  selector: 'app-cassava-other-assumptions-section',
  standalone: true,
...
export class CassavaOtherAssumptionsSectionComponent implements OnInit, OnDestroy {
  readonly sections: OtherAssumptionsSectionConfig[] = [
    {
      key: 'inflation_schedule',
      legend: 'Inflation Schedule',
      requiredColumns: ['Year', 'CPI', 'FX Index', 'Tariff Escalation'],
      priorityColumns: ['Year', 'CPI', 'FX Index', 'Tariff Escalation'],
      scrollHeight: '220px',
    },
    {
      key: 'risk_schedule',
      legend: 'Risk Schedule',
      requiredColumns: [
        'Risk',
        'Class',
        'Probability',
  ...
  isNumericColumn(column: string): boolean {
    return /year|cpi|index|escalation|probability|impact|downside|months|rate|value|%|p90/i.test(
      column,
    );
  }
  ...
  private sectionByKey(tableKey: string): OtherAssumptionsSectionConfig | undefined {
