...
export class BiotechProductVaccineMarketShareFieldsetComponent implements OnInit {
  ...
  private syncFromModel(): void {
    const stored =
      this.biotechModelService.getInputSnapshot()?.vaccineMarketShareAssumptions?.rows;
    if (Array.isArray(stored) && stored.length) {
      this.rows = stored.map((row: any) => ({
        id: String(row?.id ?? ''),
        name: String(row?.name ?? ''),
        marketType: String(row?.marketType ?? ''),
        relevantMarketSizeUsd: Number(row?.relevantMarketSizeUsd ?? 0),
        revenueTargetPatentPct: Number(row?.revenueTargetPatentPct ?? 0),
        revenueTargetPostPct: Number(row?.revenueTargetPostPct ?? 0),
