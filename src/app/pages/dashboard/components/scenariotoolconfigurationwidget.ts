...
@Component({
  standalone: true,
  selector: 'scenario-tool-configuration-widget',
  ...
  template: `
    <p-fluid class="flex">
      <div class="card w-full flex flex-col gap-4">
        <div class="text-2xl font-semibold">Scenario Tool Configuration</div>
        <div class="text-lg font-semibold">Decision Tree Tools</div>
...
export class ScenarioToolConfigurationWidget {
  form: FormGroup;
  ...
  constructor(private fb: FormBuilder) {
    this.form = this.fb.group({
      variables: this.fb.array([
        new FormControl<string>('Net Revenue'),
        new FormControl<string>('Net Income'),
      ]),
    });

    this.newVariableControl = new FormControl<string>('');
  }
