...
export class BiotechProductVaccineCapexFieldsetComponent implements OnInit {
  ...
  private syncFromModel(): void {
    const stored =
      this.biotechModelService.getInputSnapshot()?.vaccineCapexAssumptions?.rows;
    if (Array.isArray(stored) && stored.length) {
      this.rows = stored.map((row: any) => ({
        id: String(row?.id ?? ''),
        name: String(row?.name ?? ''),
        preGtmCapexSpentUsd: Number(row?.preGtmCapexSpentUsd ?? 0),
        preGtmCapexRemainingUsd: Number(row?.preGtmCapexRemainingUsd ?? 0),
        postGtmYearlyCapexUsd: Number(row?.postGtmYearlyCapexUsd ?? 0),
      }));
      this.selectedRowId = this.rows[0]?.id ?? '';
      this.syncSelectedRow();
    }
